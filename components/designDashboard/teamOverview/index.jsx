...
function TeamOverview({ designAndDieAnalytics }) {
    const [progress, setProgress] = useState(0)

    // Updated data structure based on your JSON
    const data = designAndDieAnalytics?.data?.designTeamOverview ;
            ...
            >
                <div className="grid grid-cols-3  gap-[15px] items-center ">
                    ...
                    {teamMembers?.map((member, idx) => {
                        ...
                        return (
                            <SwiperSlide key={idx}>
                                <div className={`flex items-center gap-[37px] h-full w-full border rounded-[46px] px-[40px] py-[15px] transition-all duration-300 ${
                                    !member.isOnline 
                                        ? '' 
                                        : ''
                                }`}>
